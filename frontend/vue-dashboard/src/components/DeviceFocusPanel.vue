...
const metricCards = computed(() => {
  ...
  return [
    ...
    { label: "体温", value: sample ? sample.temperature.toFixed(1) : "--", unit: "℃" },
...
const observations = computed(() => {
  ...
  if (!sample) {
    return ["请选择设备，查看单设备剖面、趋势摘要和观察建议。"];
  }
  ...
  if (sample.heart_rate > 110) {
    notes.push("心率偏高，需结合活动状态、情绪变化和近期用药进一步判断。");
  }
  if (trend.length >= 2) {
    const start = trend[0];
...
<template>
  <section class="panel focus-panel">
    <div class="panel-head">
      <div>
        <h2>单设备深描</h2>
        <p class="panel-subtitle">围绕当前选中设备，给出现场答辩更直观的生命体征剖面。</p>
      </div>
      <span>{{ riskLabel }}</span>
    </div>
    ...
    <p v-else class="empty-copy">当前没有可展示的设备剖面数据。</p>
  </section>
</template>
